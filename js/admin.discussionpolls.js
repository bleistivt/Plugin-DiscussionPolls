jQuery(($) => {
    const Closed = gdn.definition('DP_Closed'); // This determines if the poll is closed and will be used in future releases
    const QuestionString = gdn.definition('DP_EmptyQuestion'); // This is an empty form question; it is used to sidestep differences between 2.0 and 2.1 form functions
    let ExistingPoll = false; // Is this a pre-existing poll page? Assume it is a new page

    // Get strings from definitions and translated render
    const NextQString = gdn.definition('DP_NextQuestion');
    const PrevQString = gdn.definition('DP_PrevQuestion');
    const AddQString = $('#DP_NextQuestion').text();
    ...
    $('#DP_AddOption').click(function(event) {
        ...
        if (ExistingPoll) {
            ...
            const questionSet = $('fieldset.DP_Question:visible');
            // clone the labels and input
            const label = $(questionSet).children('label:last').clone();
            const inputWrapper = $(questionSet).children('.TextBoxWrapper:last').clone();

            // find the option number
            let optionNum = $(inputWrapper).children().attr('id').replace(/DP_Options\d+\.(\d+)/g, '$1');
            optionNum++;
    ...
    $('#DP_NextQuestion').click((event) => {
        ...
        if (ExistingPoll) {
            ...
            if ($('#DP_Question' + nextQuestionNum).length === 0) {
                ...
                $('#DP_Question' + questionNum).fadeOut(400, () => {
            ...
            if ($('#DP_Question' + nextQuestionNum).length === 0) {
                ...
                const newQuestion = $(QuestionString).insertAfter('#DP_Question' + questionNum);
    ...
    $('#DP_PreviousQuestion').click(function(event) {
        event.preventDefault();
        // find the current question
        const questionNum = $('fieldset.DP_Question:visible').attr('id').replace(/DP_Question(\d+)/g, '$1');
